...
def reset_rarity():
    ...
    if best_rarity_value > 0:
        # Only reset the best rarity
        luck_points += best_rarity_value
        best_rarity_value = 0
    roll_history_display()
    update_status()

def prestige():
    global prestige_points, best_rarity_value, luck_points
    if best_rarity_value >= 15:
        # Reset all lower tiers
        prestige_points += 1
        best_rarity_value = 0
        luck_points = 0
    update_status()
...
def transcendent():
    global transcendent_points, prestige_points, best_rarity_value, luck_points
    if best_rarity_value >= 30:
        # Reset everything below transcend
...
def update_status():
    status_label.config(text=f"Current Rarity: {current_rarity['name']}\n"
                             f"Best Rarity Value: {best_rarity_value}\n"
